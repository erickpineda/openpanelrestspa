...
export class UsuarioIdRedirectGuard implements CanActivate {
  ...
  async canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<boolean | UrlTree> {
    ...
    try {
      const resp: OpenpanelApiResponse<Usuario> = await firstValueFrom(this.usuarioService.obtenerPorId(id));
      const username = (resp?.data as any)?.username;
      if (username) {
        return this.router.createUrlTree(['/admin/control/gestion/usuarios/editar/username', username]);
      }
      return true;
    } catch {
      return true;
