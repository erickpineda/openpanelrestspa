...
export class ListadoCategoriasComponent implements OnInit, OnDestroy {
  ...
  loadStats(): void {
    this.dashboardApi.getTop('categories', 500, true).subscribe({
      next: (items: TopItemDTO[]) => {
        if (items && items.length > 0) {
          this.categoryStats.clear();
          items.forEach((item) => {
            if (item.name) this.categoryStats.set(item.name.toLowerCase(), item.count || 0);
          });
          if (this.listaCategorias.length > 0) this.applyStats();
        }
      },
      error: (err) => {
  ...
  applyStats(): void {
    ...
    if (newLista !== this.listaCategorias) this.listaCategorias = newLista;
    const newPaged = updateList(this.pagedCategorias);
    if (newPaged !== this.pagedCategorias) this.pagedCategorias = newPaged;
    if (this.allCategorias.length > 0) {
      const newAll = updateList(this.allCategorias);
      if (newAll !== this.allCategorias) this.allCategorias = newAll;
    }
