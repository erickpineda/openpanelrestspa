...
export interface ListState {
  ...
  loading: boolean;
  error: any | null;
  // Para paginación cliente vs servidor
...
export class ListadoEntradasStateService {
  ...
  setPageSize(size: number): Observable<void> {
    ...
    if (current.pageSize !== size) {
      ...
      // Si estamos en cliente, reaplicar paginación
      if (!current.isServerPaging && current.allEntradasClientCache.length > 0) {
        ...
        // Si estamos en servidor, recargar
  ...
  private processResponse(data: any, pageRequest: number) {
    ...
    // Mapear categorías (lógica de presentación movida aquí o mantener en componente? 
    // Mejor aquí para entregar datos listos)
    ...
    if (hasServerPaging) {
      ...
      this.updateState({
        ...
        isServerPaging: true,
        allEntradasClientCache: [] // Limpiar caché
      });
    } else {
