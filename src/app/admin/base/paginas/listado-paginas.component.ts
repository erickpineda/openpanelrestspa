...
export class ListadoPaginasComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.busquedaService.iniciarBusqueda(
      (term, page) => this.realizarBusquedaPaginas(term, page),
  ...
  private cargarDefinicionesBuscador(): void {
    ...
    this.entradaService.obtenerDefinicionesBuscadorSafe()
      ...
      .subscribe({
        ...
        error: (error) => {
          this.log.error('Error cargando definiciones:', error);
          this.cdr.markForCheck();
        }
      });
  }
  ...
  private realizarBusquedaPaginas(term: string, page?: number) {
    ...
    if (term && this.campoSeleccionado && this.operacionSeleccionada) {
    ...
    if (this.allEntradas.length > 0) {
      ...
      this.busquedaService.searchNow(this.valorBusqueda, this.currentPage)
        ...
        .subscribe({
          ...
          error: (error) => this.log.error('Error en búsqueda: ' + error)
        });
    }
    this.cdr.markForCheck();
  }
