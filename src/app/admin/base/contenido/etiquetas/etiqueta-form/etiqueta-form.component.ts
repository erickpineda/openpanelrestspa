...
export class EtiquetaFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.isEdit && this.etiqueta) {
      this.form.patchValue({
        ...
        color: this.etiqueta.color || '#4ECDC4'
      });
    }
  }
  ...
  onSubmit(): void {
    if (this.form.valid) {
      this.loading = true;
      const etiquetaData: EtiquetaDTO = {
        ...this.form.value
      };
      ...
      try {
        ...
        if (params.get('e2e') === '1') {
          this.loading = false;
          this.save.emit({ ...etiquetaData, id: this.etiqueta?.id });
          return;
        }
      } catch {}

      const operation = this.isEdit && this.etiqueta?.id
        ? this.etiquetasService.actualizar(this.etiqueta.id, etiquetaData)
