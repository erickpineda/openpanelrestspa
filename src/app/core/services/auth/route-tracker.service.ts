      ...
      .subscribe((event: any) => {
        try {
          ...
          if (
            ...
            this.log.info(
              'RouteTracker: ignorando ruta pública o de login/session-expired',
              url,
            );
            return;
          }
          ...
          if (!this.tokenStorage.isLoggedIn()) {
            this.log.info(
              'RouteTracker: no se guarda ruta, usuario no autenticado',
            );
            return;
          }
          ...
          if (this.postLoginRedirect.shouldIgnoreRouteSave()) {
            this.log.info(
              'RouteTracker: Ignorando guardado por post-login-handled (anti-race)',
            );
            return;
          }
          ...
          this.log.info(
            'RouteTracker: guardada ruta válida para post-login redirect',
            url,
          );
        } catch (e) {
