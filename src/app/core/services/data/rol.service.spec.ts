...
describe('RolService request shapes', () => {
  ...
  it('obtenerPrivilegios hace GET con código', () => {
    ...
    const req = httpMock.expectOne((r) => r.url.includes('/roles/obtenerPrivilegios/R1'));
  ...
  it('override endpoints por código', () => {
    ...
    const getReq = httpMock.expectOne((r) => r.url.includes('/roles/obtenerPorCodigo/R1'));
    expect(getReq.request.method).toBe('GET');
    getReq.flush({ data: {} });

    service.actualizar('R1', { codigo: 'R1' } as any).subscribe(() => {});
    const putReq = httpMock.expectOne((r) => r.url.includes('/roles/actualizarPorCodigo/R1'));
    expect(putReq.request.method).toBe('PUT');
    putReq.flush({ data: {} });

    service.borrar('R1').subscribe(() => {});
    const delReq = httpMock.expectOne((r) => r.url.includes('/roles/borrarPorCodigo/R1'));
  ...
  it('safe overrides mapean data y success', (done) => {
    ...
    const req = httpMock.expectOne((r) => r.url.includes('/roles/obtenerPorCodigo/R2'));
  ...
  it('eliminarSafe devuelve true cuando el backend confirma', (done) => {
    ...
    const req = httpMock.expectOne((r) => r.url.includes('/roles/borrarPorCodigo/R3'));
