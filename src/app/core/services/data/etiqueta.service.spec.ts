...
describe('EtiquetaService request shapes', () => {
  ...
  it('buscar adds paging params only when provided', () => {
    ...
    expect(req1.request.params.get(OPConstants.Pagination.PAGE_NO_PARAM)).toBe(
      '2',
    );
    expect(
      req1.request.params.get(OPConstants.Pagination.PAGE_SIZE_PARAM),
    ).toBe('25');
    ...
    expect(req2.request.params.get(OPConstants.Pagination.PAGE_NO_PARAM)).toBe(
      null,
    );
    expect(
      req2.request.params.get(OPConstants.Pagination.PAGE_SIZE_PARAM),
    ).toBe(null);
  ...
  it('asociar y desasociar construyen rutas correctas', () => {
    ...
    const d1 = httpMock.expectOne((r) =>
      r.url.includes('/etiquetas/desasociar/CATEGORIA/3/4'),
    );
