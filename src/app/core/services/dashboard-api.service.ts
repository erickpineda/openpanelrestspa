...
export class DashboardApiService {
  ...
  getSummary(force = false): Observable<SummaryDTO> {
    ...
    return this.getCached<SummaryDTO>(key, this.ttlSummary, () => {
      ...
      return this.http
        .get<any>(`${this.base}/summary`, { params })
        .pipe(map((r) => (r && r.data ? (r.data as SummaryDTO) : r)));
    });
  }
  ...
  getTop(
    type: 'users' | 'categories' | 'tags' = 'users',
    limit = 10,
    force = false,
    startDate?: string,
    endDate?: string,
  ...
  getStorage(): Observable<StorageDTO> {
    return this.http
      .get<any>(`${this.base}/storage`)
      .pipe(map((r) => (r && r.data ? (r.data as StorageDTO) : r)));
  }
  ...
  evictSummary() {
    this.cache.delete('dashboard:summary');
  }
  evictSeries(days?: number) {
    if (days) {
      ...
      Array.from(this.cache.keys()).forEach((k) => {
        if (k.startsWith(splitEstadoPrefix)) this.cache.delete(k);
      });
      ...
      Array.from(this.cache.keys()).forEach((k) => {
        if (k.startsWith(splitEstadoNombrePrefix)) this.cache.delete(k);
      });
    } else {
      Array.from(this.cache.keys()).forEach((k) => {
  ...
  evictTop(type?: string) {
    if (type) {
      Array.from(this.cache.keys()).forEach((k) => {
        if (k.startsWith(`dashboard:top:type:${type}`)) this.cache.delete(k);
      });
    } else {
      Array.from(this.cache.keys()).forEach((k) => {
        if (k.startsWith('dashboard:top:type:')) this.cache.delete(k);
      });
    }
  }
  evictContentStats() {
    this.cache.delete('dashboard:content-stats');
  }
}
