...
test.describe('Protección de rutas y sesión', () => {
  ...
  test('redirige /admin a /login sin renderizar contenido de admin', async ({
    page,
  }) => {
  ...
  test('no muestra modal de sesión expirada durante el chequeo previo de auth', async ({
    page,
  }) => {
    await page.goto('/#/admin', { waitUntil: 'domcontentloaded' });
  ...
  test('redirige si token caducado sin mostrar dashboard', async ({ page }) => {
    await page.addInitScript(() => {
      ...
      localStorage.setItem(
        'sync-auth-user',
        JSON.stringify({ username: 'expired' }),
      );
    });
  ...
  test('no render de admin bajo red lenta sin sesión', async ({ page }) => {
    await page.addInitScript(() => {
      try {
        ...
        localStorage.removeItem('sync-auth-user');
      } catch {}
    });
