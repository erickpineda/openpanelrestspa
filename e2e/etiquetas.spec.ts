...
test.describe('Gestión de Etiquetas', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('/#/admin/control/etiquetas?e2e=1', {
    ...
    await page.waitForSelector('[data-testid="etiquetas-list"]', {
      state: 'visible',
      timeout: 30000,
    });
    await page.addStyleTag({
      content:
        '.loading-overlay{pointer-events:none !important; opacity:0 !important;}',
    });
  });
  ...
  test('debe crear una nueva etiqueta', async ({ page }) => {
    ...
    await page.fill(
      'textarea[data-testid="input-descripcion"]',
      descripcionEtiqueta,
    );
  ...
  test('debe editar una etiqueta existente', async ({ page }) => {
    ...
    // Editar la etiqueta
    await page.click(
      `tr:has-text("${nombreOriginal}") button:has-text("Editar")`,
    );
    await page.waitForSelector('.modal.show', { state: 'visible' });
  ...
  test('debe eliminar una etiqueta', async ({ page }) => {
    ...
    await page.fill(
      'textarea[data-testid="input-descripcion"]',
      'Etiqueta de prueba para eliminar',
    );
    ...
    const handle = page.once('dialog', async (dialog) => {
      expect(dialog.message()).toContain(
        `¿Está seguro de eliminar la etiqueta "${nombreEtiqueta}"?`,
      );
      await dialog.accept();
    });
    ...
    await page.click(
      `tr:has-text("${nombreEtiqueta}") button:has-text("Eliminar")`,
    );
  ...
  test('debe validar campos requeridos al crear etiqueta', async ({ page }) => {
    ...
    await expect(
      page.locator('button[data-testid="btn-guardar"]'),
    ).toBeDisabled();
    ...
    await expect(
      page.locator('h5:has-text("Crear Nueva Etiqueta")'),
    ).toBeVisible();
  });

  test('debe validar límite de caracteres en nombre y descripción', async ({
    page,
  }) => {
    await page.click('button:has-text("Nueva Etiqueta")');
    await page.waitForSelector('.modal.show', { state: 'visible' });
    ...
    await expect(page.locator('input[data-testid="input-nombre"]')).toHaveValue(
      'A'.repeat(50),
    );
    ...
    await expect(page.locator('textarea[id="descripcion"]')).toHaveValue(
      'B'.repeat(200),
    );
  });

  test('debe mostrar contador de caracteres en descripción', async ({
    page,
  }) => {
    await page.click('button:has-text("Nueva Etiqueta")');
    await page.waitForSelector('.modal.show', { state: 'visible' });
    ...
    // Verificar contador
    await expect(
      page.locator(`text=${descripcion.length}/200 caracteres`),
    ).toBeVisible();
  });
  ...
  test('debe tener accesibilidad correcta', async ({ page }) => {
    ...
    ).toHaveAttribute('aria-label', 'Crear nueva etiqueta');
    ...
    await expect(
      page.locator('input[data-testid="input-nombre"]'),
    ).toHaveAttribute('aria-label', 'Ingrese el nombre de la etiqueta');
    await expect(
      page.locator('textarea[data-testid="input-descripcion"]'),
    ).toHaveAttribute(
      'aria-label',
      'Ingrese una descripción opcional para la etiqueta',
    );
  });
});
